...
if [ $NOTCONNECTED -gt 0 ]; then
  ...
  HASH=$(perl -MPOSIX -e '
    ...
    print $md5;
  ' -- "$CHALLENGE" "$FRITZ_PWD")
  curl -k -s "https://$FRITZ_IP/login_sid.lua" -d "response=$CHALLENGE-$HASH" -d 'username='${FRITZ_USER} | grep -o "<SID>[a-z0-9]\{16\}" | cut -d'>' -f 2 > $SIDFILE
fi
...
$WGET --no-check-certificate -qO- https://$FRITZ_IP/cgi-bin/capture_notimeout?ifaceorminor=$FRITZ_IFACE\&snaplen=\&capture=Start\&sid=$SID | tshark -i -
